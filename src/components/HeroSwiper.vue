...
const setupFirestoreListener = () => {
  ...
  unsubscribe = onSnapshot(
    ...
    async (snapshot) => {
      ...
      // Update sliders directly without caching
...
const handleImageLoaded = (slide) => {
  ...
  if (index !== -1) {
    ...
    const updatedSlide = { ...cachedSliders.value[index], loaded: true }

    // Replace the slide in the array to trigger reactivity
    cachedSliders.value.splice(index, 1, updatedSlide)
  }
...
onMounted(async () => {
  if (props.useFirestore) {
    ...
    // If not using Firestore, use provided props
